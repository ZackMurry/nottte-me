...
import {
    Paper, Typography, Card, CardContent, CardActions, IconButton, Button
...
export default function CreateNotePreview({ jwt, onCreate }) {
    ...
    const createNote = async () => {
        const validated = validateTitle(name)
        setCreationError(validated)
        if (validated !== '') {
            return
        }
                        ...
                        >
                            <IconButton style={{ padding: '1vh 1vw', maxHeight: 96, maxWidth: 96 }}>
                                <CreateIcon
                                    fontSize='large'
                                    style={{
                                        width: 72, height: 72, maxHeight: 72, color: theme.palette.primary.main
                                    }}
                                />
                            </IconButton>
                        </div>
                    ...
                    </div>
                    {
                        created && (
                            ...
                            >
                                ...
                                <div style={{
                                    margin: '0 auto', marginTop: '5vh', display: 'flex', justifyContent: 'center', maxWidth: '80%'
                                }}
                                >
                                    <input
                                        aria-label='name'
                                ...
                                <Typography
                                    variant='h6'
                                    style={{
                                        color: theme.palette.error.main, textAlign: 'center', maxWidth: '60%', margin: '1vh auto'
                                    }}
                                >
                                    {creationError}
                                </Typography>
