...
export default function UseLinkShare() {
    ...
    const getNoteIdentifier = async () => {
        ...
        if(jwt) {
            ...
            const accessResponse = await fetch(`http://localhost:8080/api/v1/shares/principal/note/${parsedNoteIdentifier.author}/${parsedNoteIdentifier.title}/access`, accessRequestOptions)
    ...
    return (
        <div>
            ...
            >
                <Typography variant='h2' style={{marginTop: '25vh', textAlign: 'center', padding: '3vh', maxWidth: '80%', margin: '0 auto'}}>
                    Get access to {noteIdentifier.title ? '"' + noteIdentifier.title + '"' : 'a note'}                    
                </Typography>
                ...
                </Typography>
                {
                    ...
                    :
                        <div>
                            <Typography style={{textAlign: 'center', fontWeight: 500, fontSize: 18}}>
                                You need to be signed in to get access to this note. <br />
                                Please either {' '}
